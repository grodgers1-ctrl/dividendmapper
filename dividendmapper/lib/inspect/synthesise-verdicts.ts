...
function describeValue(_pe: number | null, peP: number | null): string {
  if (peP === null) return 'limited valuation history available';
  if (peP >= 0.7) return 'historically expensive';
  if (peP <= 0.3) return 'historically cheap';
  return 'mid-band on valuation';
}
...
export function synthesiseVerdicts(input: {
  ...
  return {
    value: `${ticker} is ${describeValue(current.pe, percentiles.pe)}. P/E at ${pct(percentiles.pe)}; yield at ${pct(percentiles.dividend_yield)}.`,
